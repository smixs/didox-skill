...
`hasCommittent`, `hasLgota`, `hasMarks`, `oneside`. Без `status` черновики (0)
и удалённые черновики (55) в выдаче не видны; остальные статусы приходят
(наблюдены в т.ч. 5 и 5555, вопреки официальной доке, которая обещает только
1, 2, 3, 4, 6, 8, 40). Нужен конкретный статус — фильтруй явно.

Ловушки направления: статус ищется в СВОЁМ owner — отменённые входящие (5555)
видны только с `owner=0`, `docs --status 5555` без `--owner 0` отдаст ноль.
Голый API `statistics/all` без `owner` считает ВХОДЯЩИЕ (противоположно
списку, который без `owner` отдаёт исходящие) — CLI-команда `stats` поэтому
всегда передаёт owner сама (дефолт 1, как у `docs`).
...
`5555` отменён отправителем (наблюдён на входящих, `status_comment:
"Документ отменен"`; в официальной доке не описан — не путать с `55`).
