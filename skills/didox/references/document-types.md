...
- **Числа:** Count — до 6 знаков после запятой; остальные суммы/ставки — до 2.
  Точка как разделитель, без разделителей разрядов. Если итог не делится на
  Count нацело (например 80 032 000 / 24), Summa округлить до 2 знаков, а
  DeliverySum задать точным итогом: равенство Count×Summa == DeliverySum не
  проверяется, а вот 3+ знаков в Summa валидация отклоняет
  («максимальное количество знаков после точки: 2» — прецедент 03.09.2026).
