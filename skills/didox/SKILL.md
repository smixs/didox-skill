...
| Подписать документ | `scripts/didox.py sign <DOC_ID> --serial <serial>` → см. «Подпись» |
...
API недоступен (нет токена, стенд лёг) — подача руками через кабинет
didox.uz: карта интерфейса и грабли → `references/browser-fallback.md`.
...
Подпись ставит ЭЦП-ключ пользователя через приложение E-IMZO на его машине
(`scripts/eimzo_sign.py`, мост к CAPIWS `ws://127.0.0.1:64646`). Пароль ключа
вводит пользователь в окне E-IMZO — CLI его не видит и не хранит.

```bash
scripts/eimzo_sign.py keys                    # ключи: serial, ИНН, срок
scripts/didox.py sign <DOC_ID> --serial <serial>            # собрать подпись, НЕ отправлять
scripts/didox.py sign <DOC_ID> --serial <serial> --submit   # отправить в Didox
```

Без `--submit` команда только формирует подпись с timestamp и подтверждает
готовность — реальная отправка требует явного `--submit`.

Стоп-сигнал: `--submit` вызывать **только по явной команде пользователя на
конкретный документ**. Подпись ЭЦП юридически равна собственноручной и
необратима — подписанный документ не удаляется. Сам, «чтобы проверить», не
подписывать. Рационализации, все — стоп: «пользователь же хочет, чтобы ушло»,
«тестовый, подпишу и удалю» (подписанное не удаляется), «ключ его — значит
можно». Тестировать флоу — только на стенде testapi3 тестовым ключом.
